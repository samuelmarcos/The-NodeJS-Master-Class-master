...
//Lookup all checks, get their data, sent to a validator 
...
workers.performCheck = function(originalCheck){
    ...
    //Parse the hostname ant the apth out of the original chekc data
    var parsedUrl = url.parse(originalCheck.protocol+'://'+originalCheck.url,true);
...
workers.processCheckOutcome = function(originalCheckData,checkOutcome){
   ...
   _data.update('checks',newCheckData.id, newCheckData, err=>{
        if(!err){
            //Sent the new check data to the next phase in te process if needed
            if(alertWarranted){
...
workers.init = function(){
    ...
    //Call the loop so the checks will execute latter on
    workers.loop();
};
